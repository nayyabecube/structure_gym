...
class CustomerPayment(models.Model): 
	...
	purchase_id = fields.Many2one('struct.purchase', string="purchase")
	...
	def get_branch(self):
		...
		if self.partner_id:
			reg = self.env['reg.form'].search([('member_link','=',self.partner_id.id)])
			if reg:
				self.membership_no = reg.id
			else:
	...
	def get_partner(self):
		if self.membership_no:
			self.partner_id = self.membership_no.member_link.id


	def purchase_amt(self):
		if self.purchase_id:
			if self.amount > self.purchase_id.duetotal:
				raise ValidationError('Amount is not Correct')
			else:
				self.purchase_id.duetotal = self.purchase_id.duetotal - self.amount
	...
	def create_journal_entry(self):
		...
		if not self.journal_entry_id:	
			...
			if self.credit == True:
				create_credit_entry = journal_entries.create({
					'journal_id': self.journal_id.id,
					'date':self.date,
					'ref' : self.reference,						
					'branch' : self.branch.id,						
					})
				value = self.amount * self.credit_acc.credit_per / 100
				creat_debit = self.create_entry_lines(self.credit_acc.id,value,0,create_credit_entry.id)
				creat_credit = self.create_entry_lines(self.partner_id.property_account_receivable_id.id,0,value,create_credit_entry.id)
