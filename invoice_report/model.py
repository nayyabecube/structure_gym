...
class SampleDevelopmentReport(models.AbstractModel):
    ...
    def render_html(self,docids, data=None):
        ...
        def get_record(attr):
            del record[:]
            if types == 'all':
                rec = self.env['account.invoice'].search([('branch','=',attr),('state','=',record_wizard.stage_of_invoice),('date_invoice','>=',record_wizard.date_from),('date_invoice','<=',record_wizard.date_to)])
                for x in rec:
                    record.append(x)


            if types == "specfic":
                rec = self.env['account.invoice'].search([('branch','=',attr),('state','=',record_wizard.stage_of_invoice),('date_invoice','>=',record_wizard.date_from),('date_invoice','<=',record_wizard.date_to),('type_of_invoice','=',record_wizard.type_of_invoice)])
                for x in rec:
                    record.append(x)
        ...
        def get_type():
            ...
            if record_wizard.stage_of_invoice == 'open':
                value = "Open"
            if record_wizard.stage_of_invoice == 'paid':
                value = "Paid"
